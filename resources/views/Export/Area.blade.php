                ...
                @if(count($salaryHead) >= $d)
                    ...
                    @php
                        $loopValue=$loop->iteration;
                         if($loop->iteration == 1){
                            $toal1=$toal1+($s*$b['month']);
                         }elseif ($loop->iteration == 2){
                         $toal2=$toal2+($s*$b['month']);
                         }elseif ($loop->iteration == 3){
                         $toal3=$toal3+($s*$b['month']);
                         }elseif ($loop->iteration == 4){
                         $toal4=$toal4+($s*$b['month']);
                         }elseif ($loop->iteration == 5){
                         $toal5=$toal5+($s*$b['month']);
                         }elseif ($loop->iteration == 6){
                         $toal6=$toal6+($s*$b['month']);
                         }
                        $sum=$sum+($s*$b['month']);
                    @endphp
                @endif
            ...
            @foreach(json_decode($b['salary_head']) as $d=>$s)
                @if(count($salaryHead) >= $d)
                    @if($b['salary_flag'] == "Per Day")
                        <td>{{round($s*$b['month']*$b['Total'],0)}}</td>
                    @else
                        <td>{{round($s*$b['month']/$b['assignDay']*$b['Total'],0)}}</td>
                    @endif
                    @php
                        if($b['salary_flag'] == "Per Day"){
                           $wages=$wages+round($s*$b['month']*$b['Total'],0);
                                if($loop->iteration == 1){
                                 $totalWages1=$totalWages1+(round($s*$b['month']*$b['Total'],0));
                                }elseif ($loop->iteration == 2){
                                 $totalWages2=$totalWages2+(round($s*$b['month']*$b['Total'],0));
                                 }elseif ($loop->iteration == 3){
                                 $totalWages3=$totalWages3+(round($s*$b['month']*$b['Total'],0));
                                 }elseif ($loop->iteration == 4){
                                 $totalWages4=$totalWages4+(round($s*$b['month']*$b['Total'],0));
                                 }elseif ($loop->iteration == 5){
                                 $totalWages5=$totalWages5+(round($s*$b['month']*$b['Total'],0));
                                 }elseif ($loop->iteration == 6){
                                 $totalWages6=$totalWages6+(round($s*$b['month']*$b['Total'],0));
                                 }
                        }else{
                            $wages=$wages+round($s/$b->assignDay*$b->Total,0);
                           if($loop->iteration == 1){
                                 $totalWages1=$totalWages1+(round(($s*$b['month'])/$b['assignDay']*$b['Total'],0));
                                }elseif ($loop->iteration == 2){
                                 $totalWages2=$totalWages2+(round(($s*$b['month'])/$b['assignDay']*$b['Total'],0));
                                 }elseif ($loop->iteration == 3){
                                 $totalWages3=$totalWages3+(round(($s*$b['month'])/$b['assignDay']*$b['Total'],0));
                                 }elseif ($loop->iteration == 4){
                                 $totalWages4=$totalWages4+(round(($s*$b['month'])/$b['assignDay']*$b['Total'],0));
                                 }elseif ($loop->iteration == 5){
                                 $totalWages5=$totalWages5+(round(($s*$b['month'])/$b['assignDay']*$b['Total'],0));
                                 }elseif ($loop->iteration == 6){
                                 $totalWages6=$totalWages6+(round(($s*$b['month'])/$b['assignDay']*$b['Total'],0));
                                 }
                        }
                    @endphp
                @endif
